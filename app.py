...
st.title("Social Media Chat Analyser")
...
if uploaded_file is not None:
    ...
    if st.button("Visualize"):
        ...
        if selected_user != "Group":
            ...
            emotion_dict = dict(user_custom_df["emotion"].value_counts())
            fig6, ax = plt.subplots()
            for i in unique_emotions:
                if i not in emotion_dict:
                    emotion_dict[i] = 0
            neutral_dict = dict()
            neutral_dict["neutral"] = emotion_dict["neutral"]
            # emotion_dict.pop("neutral")
            indices = list(emotion_dict.keys())
            values_ = list(emotion_dict.values())
            
            ax.bar(indices, values_, color="orange")
            plt.xticks(rotation="vertical")
            st.pyplot(fig6)

            # fig7, ax = plt.subplots()
            # ax.bar(list(neutral_dict.keys()), list(neutral_dict.values()), width=0.1, color="black")
            # st.pyplot(fig7)
